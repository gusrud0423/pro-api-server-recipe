...
class RecipeResource(Resource) : 
    ...
    def put(self, recipe_id) :        
        ...
        # 유저아이디가 이 레시피 만든 유저가 맞는지 확인 해 봐야 한다
        user_id = get_jwt_identity()
...
## 그 레시피를 작성한 유저만 , 레시피를 삭제할 수 있게 변경 
    ...
    def delete(self, recipe_id) :
        ...
        # 먼저 인증 토큰에서 유저아이디를 빼온다
        user_id = get_jwt_identity()

        # 데이터 베이스에 저장된 유저아이디와 같은지 비교한다
        query = """select * from recipe where id = %s"""  # * 도 되고  user_id 도 가능
        ...
        if len(records) == 0 :   # 길이가 0 이면 레시피가 없는거다
            return {"err_code":1}, HTTPStatus.BAD_REQUEST
